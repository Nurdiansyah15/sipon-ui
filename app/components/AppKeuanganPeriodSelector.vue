...
const { selectedPeriodId, selectedPeriod, setPeriod, clearPeriod, loadPeriods, periods } = useKeuanganPeriodContext()
...
const label = computed(() => {
  if (selectedPeriod.value) {
    return selectedPeriod.value.name
  }
  return 'Pilih Periode Akuntansi'
})
...
<template>
  <div class="flex items-center">
    <UTooltip
      v-if="empty"
      text="Belum ada periode akuntansi. Buat di Master › Periode Akuntansi."
      :content="{ side: 'bottom' }"
    >
      <span
