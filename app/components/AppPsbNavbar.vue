...
const links = computed(() => {
  const items: { label: string, icon: string, to: string }[] = [
    { label: 'Dasbor', icon: 'i-lucide-gauge', to: '/admin/psb' },
  ]
  if (can('manage_psb')) {
    items.push({ label: 'Kelola Pendaftar', icon: 'i-lucide-users', to: '/admin/psb/pendaftaran' })
  }
  if (can('manage_psb_settings')) {
    items.push({ label: 'Setting PSB', icon: 'i-lucide-calendar-range', to: '/admin/psb/settings' })
  }
  return items
})

function isActive(to: string) {
  if (to === '/admin/psb') return route.path === '/admin/psb'
...
<template>
  <nav class="sticky top-0 z-50 border-b border-gray-200 bg-white dark:border-gray-700/50 dark:bg-gray-900">
    <div class="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3">
      ...
      <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">PSB</span>
