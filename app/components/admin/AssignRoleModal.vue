  ...
  async (open) => {
    if (open) {
      ...
      if (store.roles.length === 0) {
        store.fetchRoles({ limit: 100, page: 1 }).catch(() => {});
      }
    }
  },
);
  ...
  try {
    await store.assignUserRole({
      ...
      expired_at: event.data.expired_at
        ? new Date(event.data.expired_at).toISOString()
        : null,
    });
    ...
    <template #content>
      <div class="p-6">
        ...
        <div class="mb-6">
          ...
          <div
            v-else
            class="divide-y rounded-lg border border-gray-200 dark:border-gray-700/50"
          >
            <div
            ...
            >
              <div class="flex items-center gap-2">
                <span
                  ...
                  >{{ assignment.role.display_name }}</span
                >
                <UBadge
                ...
                <UBadge
                  :color="scopeBadgeColor(assignment.scope_type)"
                  variant="subtle"
                  size="xs"
                >
        ...
        <!-- Add role form -->
        <div
          class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700/50 dark:bg-gray-800"
        >
