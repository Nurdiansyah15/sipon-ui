...
const { selectedPeriodId, loadPeriods, periods } = useKeuanganPeriodContext()
...
// Periode akuntansi otomatis mengikuti periode kerja yang aktif; dropdown
// dinonaktifkan (auto ambil dari periode kerja).
...
// Saat periode kerja berganti, muat ulang laporan.
watch(selectedPeriodId, () => {
  if (filterAccountId.value) loadData()
})

async function loadData() {
  if (!filterAccountId.value || !selectedPeriodId.value) return
...
function canSubmit() {
  return !!filterAccountId.value && !!selectedPeriodId.value
}
...
async function onDownloadPdf() {
  ...
  try {
    await downloadPdf(
      `/api/v1/web/keuangan/admin/reports/ledger/pdf?account_id=${filterAccountId.value}&period_id=${selectedPeriodId.value}`,
...
onMounted(async () => {
  try {
    ...
    if (filterAccountId.value && selectedPeriodId.value) {
      await loadData()
    }
  } catch {
    // error handled in store
