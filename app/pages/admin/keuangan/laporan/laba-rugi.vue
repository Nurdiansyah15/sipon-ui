...
watch(selectedPeriodId, () => loadData())

async function loadData() {
  if (!selectedPeriodId.value) return
  try {
    await reportsStore.fetchIncomeStatement({ period_id: selectedPeriodId.value })
  } catch {
...
async function onDownloadPdf() {
  ...
  try {
    await downloadPdf(
      `/api/v1/web/keuangan/admin/reports/income-statement/pdf?period_id=${selectedPeriodId.value}`,
      `laba-rugi-${selectedPeriodId.value}.pdf`,
    )
  } catch {
