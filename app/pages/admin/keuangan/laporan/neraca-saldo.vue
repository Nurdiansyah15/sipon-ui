...
async function loadData() {
  ...
  try {
    await reportsStore.fetchTrialBalance({ period_id: selectedPeriodId.value })
  } catch {
...
async function onDownloadPdf() {
  ...
  try {
    await downloadPdf(
      `/api/v1/web/keuangan/admin/reports/trial-balance/pdf?period_id=${selectedPeriodId.value}`,
      `neraca-saldo-${selectedPeriodId.value}.pdf`,
    )
  } catch {
