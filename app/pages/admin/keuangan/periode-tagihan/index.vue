...
async function load() {
  try {
    await store.fetchBillingPeriods({
      page: page.value,
      limit: limit.value,
      status: statusFilter.value && statusFilter.value !== 'all' ? (statusFilter.value as BillingPeriod['status']) : undefined,
    })
  } catch {
    // error in store
  }
...
const totalPages = computed(() => store.billingPeriodsMeta?.total_pages ?? 1)
const totalItems = computed(() => store.billingPeriodsMeta?.total ?? 0)
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-700 dark:text-gray-300">
          Kelola periode tagihan: buat, buka, dan tutup periode yang digunakan saat membuat tagihan santri.
        </p>
      </div>
      <UButton
